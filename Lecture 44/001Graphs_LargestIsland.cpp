	...
	for (int i = 0; i < m; i++) {
		for (int j = 0; j < n; j++) {
			if (grid[i][j] == 1 and !visited[i][j]) {
				// (i, j)th cell corr. to a vertex and it is not yet visited
				// so make it a src and run a dfs to find out the size of the
				// component in which (i, j)th cell is present
				maxArea = max(maxArea,
