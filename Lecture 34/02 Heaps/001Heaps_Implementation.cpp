...
class minHeap {
	...
	void heapify(int i) {
		...
		int minIdx = i; // assume node with the index i has the smallest value
		...
		if (minIdx != i) {
			// heap prop. is indeed violated at index i
			swap(v[i], v[minIdx]);
			heapify(minIdx);
		}
...
public:
	...
	void pop() {
		...
		swap(v[0], v[v.size() - 1]);
		v.pop_back(); // minVal is deleted from the minHeap
		heapify(0);
	...
	int top() {
		return v[0]; // assume heap is non-empty
	}
