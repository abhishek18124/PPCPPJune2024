...
int f2(const vector<vector<int>>& grid, int m, int n, int i, int j) {
	...
	int X = f2(grid, m, n, i, j + 1);
	int Y = f2(grid, m, n, i + 1, j);
...
int fBottomUp(const vector<vector<int>>& grid, int m, int n) {
	...
	for (int i = m - 1; i >= 0; i--) {
		for (int j = n - 1; j >= 0; j--) {
			// dp[i][j] = f(i, j) = find the minSum path from (i, j) to (m-1, n-1)
			if (i == m - 1 and j == n - 1) {
				dp[i][j] = grid[i][j];
			} else if (i == m - 1) {
	...
	for (int i = m - 1; i >= 0; i--) {
		for (int j = n - 1; j >= 0; j--) {
			...
			if (i == m - 1 and j == n - 1) {
				...
				// before update, dp[j+1] repr. curRow(j+1) and dp[j] repr. nextRow(j)
