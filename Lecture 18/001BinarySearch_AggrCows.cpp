...
bool canPlaceCows(int c, int n, int pos[], int m) {
	...
	// check if you can place c cows in n stalls such that the min-dist b/w them is >= m
...
int largestMinDist(int n, int c, int pos[]) {
	...
	while (s <= e) { // log(size of search space) i.e. log(e-s)
		int m = s + (e - s) / 2;

		// can you place 'c' cows in 'n' stalls such that
		// the minimum distance b/w them is at-least 'm'?

		if (canPlaceCows(c, n, pos, m)) { // linear i.e. n steps
